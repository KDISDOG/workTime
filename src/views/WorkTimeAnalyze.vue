            ...
            <div v-for="pyrdItem in pyrdList" :key="pyrdItem.pyrd"
                ...
                <div class="bg-gray-900 text-white px-6 py-4">
                    <h3 class="text-xl font-bold flex justify-between items-center">
                        ...
                        <ChevronDownIcon class="size-6" v-if="!showStatus[pyrdItem.pyrd]"
                            @click="showSR(pyrdItem.pyrd)">
                        </ChevronDownIcon>
                ...
                <!-- SR層級 -->
                <div v-show="showStatus[pyrdItem.pyrd]" v-for="srItem in objectToArray(pyrdItem.srGroups)"
                    :key="srItem.SR" class="mx-4 mb-6 bg-gray-50 rounded-md overflow-hidden shadow">
                    ...
                    <div v-show="showStatus[srItem.SR]" v-for="taskItem in objectToArray(srItem.taskGroups)"
                        ...
                        <div
                            ...
                            <h5 class="font-bold text-[#505887]">
                                任務ID: {{ taskItem.taskId + taskItem.taskname }}
                            </h5>
                            <div class="flex items-center gap-3 min-w-[400px]">
                                <span class="flex-1 text-nowrap text-sm font-bold text-[#68748d]">最低工時: {{
                                    isOverdue(taskItem.limitTime) || "沒資料" }}</span>
                                <span class="flex-1 text-nowrap text-sm font-extrabold text-[#505887]">目前工時: {{
                                    taskItem.totalTime || "沒資料" }}</span>
                                <span class="flex-1 text-nowrap text-sm font-bold text-[#ad6666]">限制工時: {{
                                    taskItem.limitTime || "沒資料" }}</span>
                                <ChevronDownIcon class="size-6 text-black" v-if="!showStatus[taskItem.taskId]"
                                    @click="showSR(taskItem.taskId)">
                                </ChevronDownIcon>
                                <ChevronUpIcon class="size-6 text-black" v-else @click="showSR(taskItem.taskId)">
                                </ChevronUpIcon>
                            </div>
                        </div>

                        <!-- 詳細記錄表 -->
                        <div v-show="showStatus[taskItem.taskId]"
                            class="overflow-x-auto rounded-b-md border border-gray-200">
...
const fetchTimeReport = async () => {
    try {
        ...
        const response = await axios.get("/api/worktime/timeReport");
